...
template <typename T> auto have_star_equal(const T& x, size_t n) -> decltype(T(x) *= n, std::true_type());
std::false_type have_star_equal(...);

template <typename T> auto have_star(const T& x, size_t n) -> decltype(x * n, std::true_type());
std::false_type have_star(...);

template <typename T> auto have_plus(const T& x) -> decltype(x + x, std::true_type());
